...
void process_mode() {
    ...
    // Send termination signal to observer processes
    printf("All voters completed. Terminating observers...\n");
    for (int i = 0; i < num_observers; i++) {
        kill(observer_pids[i], SIGTERM);
    }
    
    // Wait for observer processes to terminate
    for (int i = 0; i < num_observers; i++) {
        int status;
